...
int main(void) {
	...
	while(1) {
		...
		if(FD_ISSET(listenfd , &rset))
		{
			...
			if(--nready <= 0) //当nready为1时表明只有监听描述符而没有读事件发生 下面就不用执行了
				continue;
		}//if
		/*接下来逐个处理连接描述符*/
		for(i=0 ; i<=maxi ; ++i)
...
void* handleRequest(int *fd)
{
	...
	//接收用户发送的消息
	//默认会阻塞 会一直等到客户端发送消息，适合模拟多线程处理
	//返回读取缓冲区的长度
	n = recv(sockfd , &buf , sizeof(buf)+1 , 0);
	//memcpy(&message , buf , sizeof(buf));
	
	printf("客户端发送的消息为：%p, n = %d\n", buf, n);
	if(n <= 0)
	{
		//关闭当前描述符，并清空描述符数组 
		fflush(stdout);
		...
		*fd = -1;
		...
		memcpy(&message , buf , min(sizeof(buf), sizeof(message)));		
		/*为每个客户操作链接创建一个线程*/				
		switch(message.msgType)
		{
			case REGISTER:{
				printf("来自%s的注册请求！, %d\n", inet_ntoa(message.sendAddr.sin_addr), __LINE__);
