...
//预订义数据库名称
#define DB_NAME "/usr/local/sqlite/bin/chatRome.db"
...
typedef struct _Message {
	char content[2048];          //针对聊天系统类型的消息
	int msgType;                 //消息类型 为MessageType中的值
	int msgRet;                  //为StateRet中的值
	struct sockaddr_in sendAddr; //发送者的ip
	struct sockaddr_in recvAddr; //接收者的ip
	char sendName[20];           //发送者的名称
	char recvName[20];			 //接收者的名称
